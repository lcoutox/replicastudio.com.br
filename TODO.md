...
- [x] **Sala de apuração** (2026-09-12, branch `redesign-ui-ux`). Substitui o fluxo `apuracao/` (Markdown + Claude Code) do repo `replica` — nasce automaticamente quando uma Pauta do radar muda pra status "apuração". Fontes anexáveis (link, arquivo via R2, ou nota sem fonte formal — visualmente distinta, mesma regra de "não publicar fato sem fonte"), conversa com agente (Sonnet + busca web nativa da Anthropic, só quando pedido), dossiê central que só muda quando o humano aceita/edita — o agente nunca escreve nele sozinho. Testado de ponta a ponta com a pauta real do poço de Ripas: o agente achou uma matéria real, cruzou com uma nota informal anexada e apontou uma discrepância de datas real — comportamento exatamente do tipo que a sala deveria ter.
